...
export function ThemeProvider({ children, preferenceStore }: { children: ReactNode; preferenceStore?: ThemePreferenceStore }) {
  ...
  useEffect(() => { applyTheme(preference); const media = window.matchMedia("(prefers-color-scheme: dark)"); const updateSystemTheme = () => { if (preference === "system") applyTheme("system"); }; media.addEventListener("change", updateSystemTheme); return () => { media.removeEventListener("change", updateSystemTheme); }; }, [preference]);
  useEffect(() => {
