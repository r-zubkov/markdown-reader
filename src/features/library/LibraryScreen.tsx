...
export function LibraryScreen({ repository, workerFactory = createImportWorker }: LibraryScreenProps) {
  ...
  return <main className="screen screen--library" id="main-content" tabIndex={-1}>
    <section aria-labelledby="library-title" className="screen__content">
      ...
      <LibraryList library={library} onImport={() => { setImportOpen(true); }} onRequestDelete={setDeleteDocument} onRetry={() => { void retryLibrary(); }} />
    </section>
...
function LibraryList({ library, onImport, onRequestDelete, onRetry }: { readonly library: LibraryState; readonly onImport: () => void; readonly onRequestDelete: (document: DocumentSummary) => void; readonly onRetry: () => void }) {
  ...
  return <ul aria-label={appCopy.library.title} className="library-list">{library.documents.map((item) => <DocumentItem document={item} key={item.documentId} onRequestDelete={onRequestDelete} />)}</ul>;
}

function DocumentItem({ document, onRequestDelete }: { readonly document: DocumentSummary; readonly onRequestDelete: (document: DocumentSummary) => void }) {
