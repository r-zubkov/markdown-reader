...
export const importCopy = {
  ...
  decisions: {
    title: "Проверяем совпадения",
    exactDescription: "Такой Markdown-файл уже есть в библиотеке. Новый документ не был создан.",
    openExisting: (title: string) => `Открыть существующий документ: ${title}`,
    updateDescription: (title: string, fileName: string) => `Файл «${fileName}» может обновить документ «${title}». Содержимое не сравнивается автоматически.`,
    oneCandidate: "Найдена одна возможная версия для обновления. Выберите дальнейшее действие.",
    manyCandidates: "Найдено несколько возможных версий. Сначала выберите документ.",
    candidateLabel: "Документ для возможного обновления",
    addSeparately: "Добавить отдельно",
    replace: "Заменить документ",
    explicitTitle: (title: string) => `Заменить «${title}»`,
    replaceExact: "Документ заменён. Место чтения перенесено точно.",
    replaceApproximate: "Документ заменён. Место чтения перенесено приблизительно; Reader покажет пояснение.",
    replaceNone: "Документ заменён, но прежнее место чтения не найдено. Reader откроется с начала.",
    cleanupPending: "Новая версия уже активна, но старые локальные данные пока не удалены. Повторите очистку.",
  ...
  errors: {
    ...
    CLEANUP_FAILED: "Не удалось удалить данные прежней версии.",
