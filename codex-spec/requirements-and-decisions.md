...
| ID | Update | Status |
|---|---|---|
| DFR-001 | P02-T01 accepts `maxFileBytes=1_250_000`, `maxAstNodes=100_000`, `maxAstDepth=128`, `maxTopLevelBlocks=20_000`, `maxTitleChars=240`, content/chunk/highlight/image limits and `8` chunks/`64_000` HTML bytes per batch; the 20,000-block boundary and production corpus were rerun in `docs/benchmarks/production-pipeline.md`. P00-T04 separately accepted `overscan=8`, cache `96`, mounted budget `48`, stabilized-drift tolerance `96px` and blank-gap tolerance `100ms`. P00-T05 adds zero-block exact tolerance, at-most-one-block approximate corpus tolerance and the `0.20` structural-similarity gate. | Content pipeline and virtual-reader portions closed. The cross-version similarity gate still requires its scheduled production update-corpus rerun. |
