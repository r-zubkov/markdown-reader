...
| ID | Update | Status |
|---|---|---|
| DFR-001 | P02-T01 accepts `maxFileBytes=1_250_000`, `maxAstNodes=100_000`, `maxAstDepth=128`, `maxTopLevelBlocks=20_000`, `maxTitleChars=240`, content/chunk/highlight/image limits and `8` chunks/`64_000` HTML bytes per batch; the 20,000-block boundary and production corpus were rerun in `docs/benchmarks/production-pipeline.md`. P00-T04 separately accepted `overscan=8`, cache `96`, mounted budget `48`, stabilized-drift tolerance `96px` and blank-gap tolerance `100ms`. P00-T05 adds zero-block exact tolerance, at-most-one-block approximate corpus tolerance and the `0.20` structural-similarity gate. P04-T02 reruns all eight update pairs through the production pipeline, repository validation and atomic ReaderState switch with no confidence or target miss. | Closed for the MVP content, virtual-reader and cross-version mapping policies; a future pipeline or corpus-policy change must rerun the corresponding evidence. |
| DFR-002 | P02-T01 accepts the explicit lowlight set `bash`, `c`, `cpp`, `csharp`, `css`, `diff`, `go`, `graphql`, `ini`, `java`, `javascript`, `json`, `kotlin`, `less`, `lua`, `makefile`, `markdown`, `objectivec`, `perl`, `php`, `plaintext`, `python`, `r`, `ruby`, `rust`, `scss`, `shell`, `sql`, `swift`, `typescript`, `wasm`, `xml`, `yaml` and the documented alias map. Auto-detect is limited to 1,200 characters and relevance at least 8; measured low/medium-confidence samples remain plain. | Closed for MVP by the production corpus/benchmark rerun. |
| PRD-008 | P03-T03 accepts `AUTO_SECTIONS_COST_THRESHOLD=24,000` as the inclusive total persisted chunk-cost boundary: at or below starts continuous, above starts sections. The pipeline-v4 corpus measurement and regression matrix are recorded in `docs/benchmarks/sections-mode-threshold.md`. | Closed for the initial-mode policy; later pipeline-cost changes must rerun the measurement before modifying the threshold. |
| PRD-003/011, TECH-012, UX-006/007 | P03-T04 accepts a 72 px semantic reading line, 120 ms coalesced UI updates, 800 ms trailing repository writes and a 90% final-block threshold for semantic 100%. Exact same-version restoration requires the unique persisted block ID; approximate/none states remain explicit. Library summaries read the same persisted ReaderState. Details are in `docs/benchmarks/progress-persistence.md`. | Closes P03 semantic progress persistence. P04-T02 must reuse the mapper/confidence contract for cross-version replacement. |
...
| PRD-013, TECH-009/010, NFR-002/007, UX-004/006 | P04-T02 captures the expected current version and ReaderState, stages the complete target, maps only validated fingerprint-capable metadata, and atomically switches Document plus mapped ReaderState. Exact/approximate/none results are visible before Open; approximate/none persist a version-scoped notice consumed once by Reader. Old-version cleanup is post-commit, idempotent and retryable. Dexie schema version 4 validates the optional notice. | Closes mapped replace/separate lifecycle behavior; P04-T03 remains responsible for the broader Reader recovery/accessibility hardening pass. |
